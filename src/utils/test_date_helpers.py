...
class TestGetDayOfWeek:
    ...
    def test_get_day_of_week_across_timezones(self):
        ...
        utc_dt = datetime(2025, 9, 27, 4, 0, 0, tzinfo=ZoneInfo('UTC'))  # Saturday 4am UTC
        toronto_dt = utc_dt.astimezone(ZoneInfo('America/Toronto'))  # Saturday midnight Toronto
...
class TestSpansMultipleDays:
    ...
    def test_spans_multiple_days_timezone_aware(self):
        ...
        start = datetime(2025, 9, 27, 23, 0, 0, tzinfo=ZoneInfo('America/Toronto'))
        end = datetime(2025, 9, 28, 6, 0, 0, tzinfo=ZoneInfo('UTC'))  # 6am UTC = 2am Toronto
        # Convert end to same timezone as start for comparison
        end_toronto = end.astimezone(ZoneInfo('America/Toronto'))
        assert spans_multiple_days(start, end_toronto) == True  # Different days in Toronto timezone
...
class TestAddComputedFields:
    ...
    def test_add_computed_fields_invalid_event_raises_error(self):
        ...
        with pytest.raises(ValueError, match="event must have 'start' and 'end' fields"):
            add_computed_fields({'id': 'test'})

        with pytest.raises(ValueError, match="start and end must have 'dateTime' fields"):
            add_computed_fields({
    ...
    def test_add_computed_fields_timezone_edge_cases(self):
        ...
        assert computed['duration'] == '2 hours'  # Raw ISO parsing shows 2 hour difference
